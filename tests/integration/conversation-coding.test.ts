...
it.each(["claude", "pi"])(
	"returns %s execution evidence to the same conversation and continues the original worktree after restart",
	async (backend) => {
		...
		try {
			...
			const response = await request(`${conversationPath}/messages`, {
				mode: "coding",
				constraints: ["Keep original interface"],
				message: "修改 value.txt",
				repositoryId: "fixture",
				runtimeId: `runtime-${backend}`,
			});
			expect(response.status).toBe(200);
			const initial = (await response.json()) as { tasks: { id: string }[] };
			const taskId = initial.tasks[0]?.id;
			let parentId = "";
			await vi.waitFor(
				async () => {
					const snapshot = (await (await request(conversationPath)).json()) as {
						tasks: { executionId: string; state: string; evidence: string[] }[];
					};
					expect(snapshot.tasks[0]?.state).toBe("completed");
					expect(snapshot.tasks[0]?.evidence.length).toBeGreaterThan(0);
					parentId = snapshot.tasks[0]?.executionId ?? "";
				},
				{ timeout: 10000 },
			);
			let childId = "";
			...
			const continued = await request(`${conversationPath}/messages`, {
				message: "继续修改",
				mode: "continue",
				taskId,
			});
			...
			await vi.waitFor(
				async () => {
					...
					expect(snapshot.tasks[0]?.state).toBe("completed");
					expect(snapshot.tasks[0]?.executionId).toBe(parentId);
					expect(snapshot.tasks[0]?.decisions).toContain("继续修改");
					expect(
						snapshot.messages.filter((m) => m.kind === "result"),
					).toHaveLength(2);
				},
				{ timeout: 10000 },
			);
